 ...
 *
 * \since This macro is available since PhysicsFS 3.3.0.
 */
#define PHYSFS_CALL
 ...
 *
 * \since This datatype is available since PhysicsFS 1.0.0.
 */
...
#ifndef PHYSFS_WIKI_DOCUMENTATION_SECTION
...
#endif  /* !PHYSFS_WIKI_DOCUMENTATION_SECTION */
 ...
 *
 * \since This struct is available since PhysicsFS 1.0.0.
 *
 * \sa PHYSFS_openRead
 * \sa PHYSFS_openWrite
 ...
 *
 * \deprecated New code should use PHYSFS_File, as this #define may go away
 *             someday.
 *
 * \since This macro is available since PhysicsFS 1.0.0.
 *
 * \sa PHYSFS_File
 */
...
/**
 * The current major version of the PhysicsFS headers.
 *
 * If this were PhysicsFS version 3.2.1, this value would be 3.
 *
 * \since This macro is available since PhysicsFS 1.0.0.
 */
...
/**
 * The current minor version of the PhysicsFS headers.
 *
 * If this were PhysicsFS version 3.2.1, this value would be 2.
 *
 * \since This macro is available since PhysicsFS 1.0.0.
 */
...
/**
 * The current patch (or micro) version of the PhysicsFS headers.
 *
 * If this were PhysicsFS version 3.2.1, this value would be 1.
 *
 * \since This macro is available since PhysicsFS 1.0.0.
 */
 ...
 * determined with PHYSFS_getLinkedVersion(), which, unlike PHYSFS_VERSION, is
 * not a macro.
 *
 ...
 *
 * This is a real function; the macro PHYSFS_VERSION tells you what version of
 * PhysFS you compiled against:
 *
 * ```c
 ...
 *
 * \param ver the PHYSFS_Version struct to fill in.
 *
 ...
 * \sa PHYSFS_VERSION
 */
extern PHYSFS_DECL void PHYSFS_CALL PHYSFS_getLinkedVersion(PHYSFS_Version *ver);
 ...
 * main() function), but you should always try to pass something in here when
 * reasonable. Many Unix-like systems _need_ to pass argv[0] from main() in
 * here. However several platforms have special needs:
 *
 * - On Android, argv0 should be a non-NULL pointer to a PHYSFS_AndroidInit
 *   struct. This struct must hold a valid JNIEnv * and a JNI jobject of a
 *   Context (either the application context or the current Activity is fine).
 *   Both are cast to a void * so we don't need jni.h included wherever
 *   physfs.h is. PhysicsFS uses these objects to query some system details.
 *   PhysicsFS does not hold a reference to the JNIEnv or Context past the
 *   call to PHYSFS_init(). If you pass a NULL here, PHYSFS_init can still
 *   succeed, but PHYSFS_getBaseDir() and PHYSFS_getPrefDir() will be
 *   incorrect.
 * - On Playdate, argv0 should be a non-NULL pointer to a PlaydateAPI struct.
 *   PhysicsFS uses this object for system-level access and will hold it until
 *   PHYSFS_deinit is called. If you pass a NULL here, PhysicsFS will crash.
 ...
 *
 * \param argv0 the argv[0] string passed to your program's mainline, or some
 ...
 * \sa PHYSFS_isInit
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_init(const char *argv0);
 ...
 * with the exception of any custom allocator you might have specified, which
 * survives between initializations.
 *
 * \returns nonzero on success, zero on error. Specifics of the error can be
 ...
 * \sa PHYSFS_isInit
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_deinit(void);
 ...
 *
 * The returned value is an array of pointers to PHYSFS_ArchiveInfo
 * structures, with a NULL entry to signify the end of the list:
 *
 * ```c
 ...
 * call to PHYSFS_deinit(), PHYSFS_registerArchiver(), or
 ...
 * \sa PHYSFS_deregisterArchiver
 */
extern PHYSFS_DECL const PHYSFS_ArchiveInfo ** PHYSFS_CALL PHYSFS_supportedArchiveTypes(void);
 ...
 */
extern PHYSFS_DECL void PHYSFS_CALL PHYSFS_freeList(void *listVar);
 ...
 * offer a better option: use PHYSFS_getLastErrorCode() instead.
 *
 ...
 * you need both, call PHYSFS_getLastErrorCode(), then pass that value to
 ...
 * use PHYSFS_getLastErrorCode() instead of this function.
 *
 * \deprecated Use PHYSFS_getLastErrorCode() and PHYSFS_getErrorByCode()
 *             instead.
 *
 ...
 */
extern PHYSFS_DEPRECATED PHYSFS_DECL const char * PHYSFS_CALL PHYSFS_getLastError(void);
 ...
 *
 * \returns READ ONLY null-terminated string of platform's dir separator.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL const char * PHYSFS_CALL PHYSFS_getDirSeparator(void);
 ...
 */
extern PHYSFS_DECL void PHYSFS_CALL PHYSFS_permitSymbolicLinks(int allow);
 ...
 *
 * \returns Null-terminated array of null-terminated strings.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 * \since This function is available since PhysicsFS 1.0.0.
 ...
 */
extern PHYSFS_DECL char ** PHYSFS_CALL PHYSFS_getCdRomDirs(void);
 ...
 * probably want to call PHYSFS_setRoot(basedir, "/assets") after mounting to
 * make your app's actual data available directly without all the Android
 * metadata and directory offset. Note that if you passed a NULL to
 * PHYSFS_init(), you will not get the APK file here.
 *
 * \returns READ ONLY string of base dir in platform-dependent notation.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 * \since This function is available since PhysicsFS 1.0.0.
 ...
 */
extern PHYSFS_DECL const char * PHYSFS_CALL PHYSFS_getBaseDir(void);
 ...
 *
 * \returns READ ONLY string of user dir in platform-dependent notation.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 * \since This function is available since PhysicsFS 1.0.0.
 ...
 *
 * \returns READ ONLY string of write dir in platform-dependent notation, OR
 *          NULL IF NO WRITE PATH IS CURRENTLY SET.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 * \since This function is available since PhysicsFS 1.0.0.
 ...
 */
extern PHYSFS_DECL const char * PHYSFS_CALL PHYSFS_getWriteDir(void);
 ...
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_setWriteDir(const char *newDir);
 ...
 * resources by calling PHYSFS_freeList() with the returned pointer.
 *
 * \returns Null-terminated array of null-terminated strings. NULL if there
 ...
 * \sa PHYSFS_removeFromSearchPath
 */
extern PHYSFS_DECL char ** PHYSFS_CALL PHYSFS_getSearchPath(void);
 ...
 * PHYSFS_getPrefDir(organization, appName), which is created if it doesn't
 * exist.
 *
 ...
 *
 * \param organization Name of your company/group/etc to be used as a dirname,
 *                     so keep it small, and no-frills.
 * \param appName Program-specific name of your program, to separate it from
 *                other programs using PhysicsFS.
 * \param archiveExt File extension used by your program to specify an
 *                   archive. For example, Quake 3 uses "pk3", even though
 *                   they are just zipfiles. Specify NULL to not dig out
 *                   archives automatically. Do not specify the '.' char; If
 *                   you want to look for ZIP files, specify "ZIP" and not
 *                   ".ZIP" ... the archive search is case-insensitive.
 * \param includeCdRoms Non-zero to include CD-ROMs in the search path, and
 *                      (if `archiveExt` != NULL) search them for archives.
 *                      This may cause a significant amount of blocking while
 *                      discs are accessed, and if there are no discs in the
 *                      drive (or even not mounted on Unix systems), then they
 *                      may not be made available anyhow. You may want to
 *                      specify zero and handle the disc setup yourself.
 * \param archivesFirst Non-zero to prepend the archives to the search path.
 *                      Zero to append them. Ignored if !`archiveExt`.
 * \returns nonzero on success, zero on error. Use PHYSFS_getLastErrorCode()
 *          to obtain the specific error.
 *
 * \threadsafety This function is not thread safe.
 *
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_setSaneConfig(const char *organization,
                                     const char *appName,
 ...
 * \sa PHYSFS_delete
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_mkdir(const char *dirName);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_delete(const char *filename);
 ...
 * with `filename`. Specifically, PHYSFS_mountIo(), PHYSFS_mountMemory(), and
 * PHYSFS_mountHandle() will return NULL even if the filename is found in the
 * search path. Plan accordingly.
 *
 * \param filename file to look for.
 * \returns READ ONLY string of element of search path containing the the file
 *          in question. NULL if not found.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL const char * PHYSFS_CALL PHYSFS_getRealDir(const char *filename);
 ...
 * \returns Null-terminated array of null-terminated strings, or NULL for
 *          failure cases.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 ...
 * \sa PHYSFS_enumerate
 */
extern PHYSFS_DECL char ** PHYSFS_CALL PHYSFS_enumerateFiles(const char *dir);
 ...
 * \returns non-zero if filename exists. zero otherwise.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_exists(const char *fname);
 ...
 *
 * Note that entries that are symlinks are ignored if
 * PHYSFS_permitSymbolicLinks(1) hasn't been called, and opening a symlink
 * with this function will fail in such a case.
 *
 * \param filename File to open.
 * \returns A valid PhysicsFS filehandle on success, NULL on error. Use
 *          PHYSFS_getLastErrorCode() to obtain the specific error.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 ...
 * \sa PHYSFS_close
 */
extern PHYSFS_DECL PHYSFS_File * PHYSFS_CALL PHYSFS_openWrite(const char *filename);
 ...
 * \sa PHYSFS_close
 */
extern PHYSFS_DECL PHYSFS_File * PHYSFS_CALL PHYSFS_openAppend(const char *filename);
 ...
 * \sa PHYSFS_close
 */
extern PHYSFS_DECL PHYSFS_File * PHYSFS_CALL PHYSFS_openRead(const char *filename);
 ...
 * \sa PHYSFS_openAppend
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_close(PHYSFS_File *handle);
 ...
 * \returns number of objects written. PHYSFS_getLastErrorCode() can shed
 ...
 *
 * \param handle handle returned from PHYSFS_openRead().
 * \returns nonzero if EOF, zero if not.
 *
 * \threadsafety Multiple threads can not operate on the same PHYSFS_File at
 *               the same time, but they can safely operate on _different_
 *               ones simultaneously.
 ...
 * \sa PHYSFS_tell
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_eof(PHYSFS_File *handle);
 ...
 *
 * \param handle handle returned from PHYSFS_open*().
 * \returns offset in bytes from start of file. -1 if error occurred. Use
 *          PHYSFS_getLastErrorCode() to obtain the specific error.
 *
 * \threadsafety Multiple threads can not operate on the same PHYSFS_File at
 *               the same time, but they can safely operate on _different_
 ...
 * \sa PHYSFS_seek
 */
extern PHYSFS_DECL PHYSFS_sint64 PHYSFS_CALL PHYSFS_tell(PHYSFS_File *handle);
 ...
 * \sa PHYSFS_tell
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_seek(PHYSFS_File *handle, PHYSFS_uint64 pos);
 ...
 * time, then the information this function supplies could be incorrect before
 * you get it. Use with caution, or better yet, don't use at all.
 *
 * \param handle handle returned from PHYSFS_open*().
 * \returns size in bytes of the file. -1 if can't be determined.
 *
 * \threadsafety Multiple threads can not operate on the same PHYSFS_File at
 *               the same time, but they can safely operate on _different_
 *               ones simultaneously.
 ...
 * \sa PHYSFS_seek
 */
extern PHYSFS_DECL PHYSFS_sint64 PHYSFS_CALL PHYSFS_fileLength(PHYSFS_File *handle);
 ...
 * and stored in the internal buffer. Calls to PHYSFS_read() will pull from
 * this buffer until it is empty, and then refill it for more reading. Note
 * that compressed files, like ZIP archives, will decompress while buffering,
 * so this can be handy for offsetting CPU-intensive operations. The buffer
 * isn't filled until you do your next read.
 *
 ...
 * \returns nonzero if successful, zero on error.
 *
 * \threadsafety Multiple threads can not operate on the same PHYSFS_File at
 *               the same time, but they can safely operate on _different_
 *               ones simultaneously.
 ...
 * \sa PHYSFS_close
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_setBuffer(PHYSFS_File *handle, PHYSFS_uint64 bufsize);
 ...
 * \sa PHYSFS_close
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_flush(PHYSFS_File *handle);
 ...
 *
 * Take a 16-bit signed value in littleendian format and convert it to the
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL PHYSFS_sint16 PHYSFS_CALL PHYSFS_swapSLE16(PHYSFS_sint16 val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL PHYSFS_uint16 PHYSFS_CALL PHYSFS_swapULE16(PHYSFS_uint16 val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL PHYSFS_sint32 PHYSFS_CALL PHYSFS_swapSLE32(PHYSFS_sint32 val);


/**
 * Swap littleendian unsigned 32 to platform's native byte order.
 *
 * Take a 32-bit unsigned value in littleendian format and convert it to the
 * platform's native byte order.
 *
 * \param val value to convert.
 * \returns converted value.
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL PHYSFS_uint32 PHYSFS_CALL PHYSFS_swapULE32(PHYSFS_uint32 val);

/**
 * Swap littleendian signed 64 to platform's native byte order.
 *
 * Take a 64-bit signed value in littleendian format and convert it to the
 * platform's native byte order.
 *
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL PHYSFS_sint64 PHYSFS_CALL PHYSFS_swapSLE64(PHYSFS_sint64 val);


/**
 * Swap littleendian unsigned 64 to platform's native byte order.
 *
 * Take a 64-bit unsigned value in littleendian format and convert it to the
 * platform's native byte order.
 *
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL PHYSFS_uint64 PHYSFS_CALL PHYSFS_swapULE64(PHYSFS_uint64 val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL PHYSFS_sint16 PHYSFS_CALL PHYSFS_swapSBE16(PHYSFS_sint16 val);


/**
 * Swap bigendian unsigned 16 to platform's native byte order.
 *
 * Take a 16-bit unsigned value in bigendian format and convert it to the
 * platform's native byte order.
 *
 * \param val value to convert.
 * \returns converted value.
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL PHYSFS_uint16 PHYSFS_CALL PHYSFS_swapUBE16(PHYSFS_uint16 val);

/**
 * Swap bigendian signed 32 to platform's native byte order.
 *
 * Take a 32-bit signed value in bigendian format and convert it to the
 * platform's native byte order.
 *
 * \param val value to convert.
 * \returns converted value.
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL PHYSFS_sint32 PHYSFS_CALL PHYSFS_swapSBE32(PHYSFS_sint32 val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL PHYSFS_uint32 PHYSFS_CALL PHYSFS_swapUBE32(PHYSFS_uint32 val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL PHYSFS_sint64 PHYSFS_CALL PHYSFS_swapSBE64(PHYSFS_sint64 val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL PHYSFS_uint64 PHYSFS_CALL PHYSFS_swapUBE64(PHYSFS_uint64 val);


/**
 * Read and convert a signed 16-bit littleendian value.
 *
 * Convenience function. Read a signed 16-bit littleendian value from a file
 ...
 * \returns zero on failure, non-zero on success. If successful, `*val` will
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_readSLE16(PHYSFS_File *file, PHYSFS_sint16 *val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_readULE16(PHYSFS_File *file, PHYSFS_uint16 *val);


/**
 * Read and convert a signed 16-bit bigendian value.
 *
 * Convenience function. Read a signed 16-bit bigendian value from a file and
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_readSBE16(PHYSFS_File *file, PHYSFS_sint16 *val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_readUBE16(PHYSFS_File *file, PHYSFS_uint16 *val);


/**
 * Read and convert a signed 32-bit littleendian value.
 *
 * Convenience function. Read a signed 32-bit littleendian value from a file
 * and convert it to the platform's native byte order.
 *
 * \param file PhysicsFS file handle from which to read.
 * \param val pointer to where value should be stored.
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_readSLE32(PHYSFS_File *file, PHYSFS_sint32 *val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_readULE32(PHYSFS_File *file, PHYSFS_uint32 *val);


/**
 * Read and convert a signed 32-bit bigendian value.
 *
 * Convenience function. Read a signed 32-bit bigendian value from a file and
 * convert it to the platform's native byte order.
 *
 * \param file PhysicsFS file handle from which to read.
 * \param val pointer to where value should be stored.
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_readSBE32(PHYSFS_File *file, PHYSFS_sint32 *val);


/**
 * Read and convert an unsigned 32-bit bigendian value.
 *
 * Convenience function. Read an unsigned 32-bit bigendian value from a file
 * and convert it to the platform's native byte order.
 *
 * \param file PhysicsFS file handle from which to read.
 * \param val pointer to where value should be stored.
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_readUBE32(PHYSFS_File *file, PHYSFS_uint32 *val);


/**
 * Read and convert a signed 64-bit littleendian value.
 *
 * Convenience function. Read a signed 64-bit littleendian value from a file
 * and convert it to the platform's native byte order.
 *
 * **WARNING**: Remember, PHYSFS_sint64 is only 32 bits on platforms without
 * any sort of 64-bit support.
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_readSLE64(PHYSFS_File *file, PHYSFS_sint64 *val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_readULE64(PHYSFS_File *file, PHYSFS_uint64 *val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_readSBE64(PHYSFS_File *file, PHYSFS_sint64 *val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_readUBE64(PHYSFS_File *file, PHYSFS_uint64 *val);
 ...
 * \returns zero on failure, non-zero on success. On failure, you can find out
 *          what went wrong from PHYSFS_getLastErrorCode().
 *
 * \threadsafety Multiple threads can not operate on the same PHYSFS_File at
 *               the same time, but they can safely operate on _different_
 *               ones simultaneously.
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_writeSLE16(PHYSFS_File *file, PHYSFS_sint16 val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_writeULE16(PHYSFS_File *file, PHYSFS_uint16 val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_writeSBE16(PHYSFS_File *file, PHYSFS_sint16 val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_writeUBE16(PHYSFS_File *file, PHYSFS_uint16 val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_writeSLE32(PHYSFS_File *file, PHYSFS_sint32 val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_writeULE32(PHYSFS_File *file, PHYSFS_uint32 val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_writeSBE32(PHYSFS_File *file, PHYSFS_sint32 val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_writeUBE32(PHYSFS_File *file, PHYSFS_uint32 val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_writeSLE64(PHYSFS_File *file, PHYSFS_sint64 val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_writeULE64(PHYSFS_File *file, PHYSFS_uint64 val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_writeSBE64(PHYSFS_File *file, PHYSFS_sint64 val);
 ...
 * \since This function is available since PhysicsFS 1.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_writeUBE64(PHYSFS_File *file, PHYSFS_uint64 val);
 ...
 *
 * Once PHYSFS_init() returns successfully, this will return non-zero. Before
 * a successful PHYSFS_init() and after PHYSFS_deinit() returns successfully,
 * this will return zero. This function is safe to call at any time.
 *
 * \returns non-zero if library is initialized, zero if library is not.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 ...
 * \sa PHYSFS_deinit
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_isInit(void);
 ...
 * PHYSFS_permitSymbolicLinks(). If PHYSFS_permitSymbolicLinks() hasn't been
 * called since the library was last initialized, symbolic links are
 * implicitly disabled.
 *
 * \returns non-zero if symlinks are permitted, zero if not.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 * \since This function is available since PhysicsFS 2.0.0.
 ...
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_symbolicLinksPermitted(void);
 ...
 *
 * \since This struct is available since PhysicsFS 2.0.0.
 *
 ...
 * only call this function before PHYSFS_init(); if the library is
 * initialized, it'll reject your efforts to change the allocator mid-stream.
 * You may call this function after PHYSFS_deinit() if you are willing to shut
 * down the library and restart it with a new allocator; this is a safe and
 * supported operation. The allocator remains intact between deinit/init
 ...
 * \returns zero on failure, non-zero on success. This call only fails when
 *          used between PHYSFS_init() and PHYSFS_deinit() calls.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 * \since This function is available since PhysicsFS 2.0.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_setAllocator(const PHYSFS_Allocator *allocator);
 ...
 * regardless of the state of PHYSFS_permitSymbolicLinks(). That function only
 * deals with symlinks inside the mounted directory or archive.
 *
 * \param newDir directory or archive to add to the path, in
 *               platform-dependent notation.
 * \param mountPoint Location in the interpolated tree that this archive will
 *                   be "mounted", in platform-independent notation. NULL or
 *                   "" is equivalent to "/".
 * \param appendToPath nonzero to append to search path, zero to prepend.
 * \returns nonzero if added to path, zero on failure (bogus archive, dir
 *          missing, etc). Use PHYSFS_getLastErrorCode() to obtain the
 ...
 * \sa PHYSFS_mountIo
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_mount(const char *newDir,
 ...
 * PHYSFS_addToSearchPath() will report "/". The return value is READ ONLY and
 * valid until the archive is removed from the search path.
 *
 * \param dir directory or archive previously added to the path, in
 *            platform-dependent notation. This must match the string used
 *            when adding, even if your string would also reference the same
 *            file with a different string of characters.
 * \returns READ-ONLY string of mount point if added to path, NULL on failure
 ...
 * \sa PHYSFS_getMountPoint
 */
extern PHYSFS_DECL const char * PHYSFS_CALL PHYSFS_getMountPoint(const char *dir);
 ...
 *
 * \param data User-defined data pointer, passed through from the API that
 *             eventually called the callback.
 * \param str The string data about which the callback is meant to inform.
 *
 * \since This typedef is available since PhysicsFS 2.0.0.
 *
 * \sa PHYSFS_getCdRomDirsCallback
 * \sa PHYSFS_getSearchPathCallback
 */
 ...
 *              fired, and it will not contain the full path. You can recreate
 *              the fullpath with $origdir/$fname ... The file can be a
 *              subdirectory, a file, a symlink, etc.
 *
 * \since This typedef is available since PhysicsFS 2.0.0.
 *
 ...
 * Rather, it calls a function specified by the application once per detected
 * disc:
 *
 * ```c
 ...
 * \sa PHYSFS_getCdRomDirs
 */
extern PHYSFS_DECL void PHYSFS_CALL PHYSFS_getCdRomDirsCallback(PHYSFS_StringCallback c, void *d);
 ...
 * Rather, it calls a function specified by the application once per element
 * of the search path:
 *
 * ```c *
 ...
 * \sa PHYSFS_getSearchPath
 */
extern PHYSFS_DECL void PHYSFS_CALL PHYSFS_getSearchPathCallback(PHYSFS_StringCallback c, void *d);
 ...
 *             function has no way to report errors (or to have the callback
 ...
 * \returns The number of codepoints the folding produced. Between 1 and 3.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 * \since This function is available since PhysicsFS 2.1.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_caseFold(const PHYSFS_uint32 from, PHYSFS_uint32 *to);
 ...
 * \returns -1 if str1 is "less than" str2, 1 if "greater than", 0 if equal.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 * \since This function is available since PhysicsFS 2.1.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_utf8stricmp(const char *str1, const char *str2);
 ...
 *
 * This is a strcasecmp/stricmp replacement that expects both strings to be in
 * UCS-4 (aka UTF-32) encoding. It will do "case folding" to decide if the
 ...
 *
 * \since This enum is available since PhysicsFS 2.1.0.
 *
 ...
 *              the fullpath with $origdir/$fname ... The file can be a
 *              subdirectory, a file, a symlink, etc.
 * \returns A value from PHYSFS_EnumerateCallbackResult. All other values are
 *          (currently) undefined; don't use them.
 *
 * \since This typedef is available since PhysicsFS 2.1.0.
 *
 * \sa PHYSFS_enumerate
...
/**
 * Get a file listing of a search path's directory, using an
 * application-defined callback, with errors reported.
 *
 * Internally, PHYSFS_enumerateFiles() just calls this function and then
 * builds a list before returning to the application, so functionality is
 * identical except for how the information is represented to the application.
 *
 ...
 * you should probably use PHYSFS_enumerateFiles() instead, which guarantees
 ...
 * PHYSFS_addToSearchPath or any of the mount functions), not the path where
 * it is mounted in the tree (the "mountPoint" argument to any of the mount
 * functions).
 *
 * \param oldDir dir/archive to remove.
 * \returns nonzero on success, zero on failure. Use PHYSFS_getLastErrorCode()
 ...
 * \sa PHYSFS_mount
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_unmount(const char *oldDir);
 ...
 * own.
 *
 * This call is only valid between PHYSFS_init() and PHYSFS_deinit() calls; it
 * will return NULL if the library isn't initialized. As we can't guarantee
 * the state of the internal allocators unless the library is initialized, you
 * shouldn't use any allocator returned here after a call to PHYSFS_deinit().
 *
 ...
 * safely ignore it altogether.
 *
 * \returns Current allocator, as set by PHYSFS_setAllocator(), or PhysicsFS's
 ...
 * \sa PHYSFS_setAllocator
 */
extern PHYSFS_DECL const PHYSFS_Allocator * PHYSFS_CALL PHYSFS_getAllocator(void);
 ...
 * Jan 1, 1970), or -1 if the information isn't available or applicable. The
 * `filesize` field is measured in bytes. The `readonly` field tells you
 * whether the archive thinks a file is not writable, but tends to be only an
 * estimate (for example, your write dir might overlap with a .zip file,
 * meaning you _can_ successfully open that path for writing, as it gets
 * created elsewhere.
 *
 * \since This struct is available since PhysicsFS 2.1.0.
 *
 * \sa PHYSFS_stat
 * \sa PHYSFS_FileType
 */
 ...
 * \returns non-zero on success, zero on failure. On failure, `stat`'s
 ...
 * \sa PHYSFS_Stat
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_stat(const char *fname, PHYSFS_Stat *stat);
 ...
 * \returns number of bytes read. This may be less than `len`; this does not
 ...
 * \returns number of bytes written. This may be less than `len`; in the case
 *          of an error, the system may try to write as many bytes as
 *          possible, so an incomplete write might occur.
 *          PHYSFS_getLastErrorCode() can shed light on the reason this might
 *          be < `len`. -1 if complete failure.
 *
 * \threadsafety Multiple threads can not operate on the same PHYSFS_File at
 *               the same time, but they can safely operate on _different_
 *               ones simultaneously.
 ...
 *
 * \threadsafety PHYSFS_Io implementations are not guaranteed to be thread
 *               safe in themselves. Under the hood where PhysicsFS uses them,
 *               the library provides its own locks. If you plan to use them
 *               directly from separate threads, you should either use mutexes
 *               to protect them, or don't use the same PHYSFS_Io from two
 ...
 * using PHYSFS_mount() instead of this.
 *
 ...
 *                   "" is equivalent to "/".
 * \param appendToPath nonzero to append to search path, zero to prepend.
 * \returns nonzero if added to path, zero on failure (bogus archive, stream
 *          i/o issue, etc). Use PHYSFS_getLastErrorCode() to obtain the
 *          specific error.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 ...
 *                   "" is equivalent to "/".
 * \param appendToPath nonzero to append to search path, zero to prepend.
 * \returns nonzero if added to path, zero on failure (bogus archive, etc).
 ...
 * seek even when the data is compressed, it may do so by rewinding to the
 * start and decompressing everything before the seek point. Normal archive
 * usage may do a lot of seeking behind the scenes. As such, you might find
 * normal archive usage extremely painful if mounted this way. Plan
 * accordingly: if you, say, have a self-extracting .zip file, and want to
 * mount something in it, compress the contents of the inner archive and make
 * sure the outer .zip file doesn't compress the inner archive too.
 *
 ...
 * archive, and is used instead of a real file in the physical filesystem. The
 * PHYSFS_File may be backed by a real file in the physical filesystem, but
 * isn't necessarily. The most popular use for this is likely to mount
 ...
 * unmounted, the system will call PHYSFS_close(file). If you need this handle
 * to survive, you will have to wrap this in a PHYSFS_Io and use
 ...
 * function. Each thread has a unique error state associated with it, but each
 * time a new error message is set, it will overwrite the previous one
 * associated with that thread. It is safe to call this function at anytime,
 * even before PHYSFS_init().
 *
 ...
 *
 * \returns Enumeration value that represents last reported error.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 * \since This function is available since PhysicsFS 2.1.0.
 ...
 * \sa PHYSFS_getErrorByCode
 */
extern PHYSFS_DECL PHYSFS_ErrorCode PHYSFS_CALL PHYSFS_getLastErrorCode(void);
 ...
 * \returns READ ONLY string of requested error message, NULL if this is not a
 *          valid PhysicsFS error code. Always check for NULL if you might be
 *          looking up an error code that didn't exist in an earlier version
 *          of PhysicsFS.
 *
 * \threadsafety It is safe to call this function from any thread.
 *
 ...
 * \sa PHYSFS_getLastErrorCode
 */
extern PHYSFS_DECL const char * PHYSFS_CALL PHYSFS_getErrorByCode(PHYSFS_ErrorCode code);
 ...
 * \sa PHYSFS_getErrorByCode
 */
extern PHYSFS_DECL void PHYSFS_CALL PHYSFS_setErrorCode(PHYSFS_ErrorCode code);
 ...
 * only PHYSFS_getUserDir() and left you to figure out where the files should
 * go under that tree. This finds the correct location for whatever platform,
 * which not only changes between operating systems, but also versions of the
 ...
 * to write files (and that PHYSFS_getUserDir() and PHYSFS_getBaseDir(), while
 * they might be writable, or even parents of the returned path, aren't where
 * you should be writing things).
 *
 * \param org The name of your organization.
 * \param app The name of your application.
 * \returns READ ONLY string of user dir in platform-dependent notation. NULL
 ...
 */
extern PHYSFS_DECL const char * PHYSFS_CALL PHYSFS_getPrefDir(const char *org, const char *app);
 ...
 * types.
 *
 * Conceptually, a PHYSFS_Archiver provides directory entries, while PHYSFS_Io
 * provides data streams for those directory entries. The most obvious use of
 * PHYSFS_Archiver is to provide support for an archive file type that isn't
 * provided by PhysicsFS directly: perhaps some proprietary format that only
 ...
 * it will be available for use automatically with PHYSFS_mount(); if a given
 * archive can be handled with your archiver, it will be given control as
 * appropriate.
 *
 ...
 * such, your PHYSFS_Archiver can assume that locking is handled for you so
 * long as the PHYSFS_Io you return from PHYSFS_open* doesn't change any of
 * your Archiver state, as the PHYSFS_Io won't be as aggressively protected.
 *
 * \since This struct is available since PhysicsFS 2.1.0.
 *
 * \sa PHYSFS_registerArchiver
 ...
 * PHYSFS_mount() to add archives that your archiver supports to the search
 * path, or perhaps use it as the write dir. Internally, PhysicsFS uses this
 * function to register its own built-in archivers, like .zip support, etc.
 *
 ...
 * \sa PHYSFS_deregisterArchiver
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_registerArchiver(const PHYSFS_Archiver *archiver);
 ...
 * mechanism to recover them, short of calling PHYSFS_deinit() and
 * PHYSFS_init() again.
 *
 ...
 * \sa PHYSFS_registerArchiver
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_deregisterArchiver(const char *ext);
 ...
 * \threadsafety It is safe to call this function from any thread.
 *
 * \since This function is available since PhysicsFS 3.1.0.
 */
extern PHYSFS_DECL int PHYSFS_CALL PHYSFS_setRoot(const char *archive, const char *subdir);
